...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && (url.pathname === "/" || url.pathname === "/api/estimate")) {
      return json(request, env, { ok: true, service: env.BUSINESS_NAME || "Go Direct Home Services Estimate Worker" });
    }
    ...
    try {
      ...
      const submission = contentType.includes("multipart/form-data")
        ...
        : await parseJsonSubmission(request);
...
async function parseJsonSubmission(request) {
